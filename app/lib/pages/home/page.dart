...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.paused) {
      ...
      if (mounted && PlatformService.isApple && !Env.isOfflineRuntime) {
  ...
  void _checkForAnnouncements() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      final deviceProvider = Provider.of<DeviceProvider>(context, listen: false);
      if (Env.isOfflineRuntime) {
        deviceProvider.onDeviceConnected = _checkDeviceOnboarding;
  ...
  @override
  Widget build(BuildContext context) {
    final product = Consumer<ConnectivityProvider>(
    ...
    if (Env.isOfflineRuntime) return product;
