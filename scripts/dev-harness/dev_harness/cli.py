...
def _opus_runtime_present(cfg: config.HarnessConfig) -> bool:
    """Exercise the decoder in the exact child environment used by backend."""
        ...
        return (
            subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "import opuslib; opuslib.Decoder(16000, 1)",
                ],
...
def prerequisite_report(cfg: config.HarnessConfig) -> tuple[list[str], list[str]]:
    ...
    if not _opus_runtime_present(cfg):
        missing.append(
            "python-opuslib with a usable native libopus (required for Omi CV1 audio; install opus and backend requirements)"
        )
    ...
    if cfg.provider_mode == "offline":
        warnings.append(
            "PROVIDER_MODE=offline: egress guard is enforced; only Firebase emulators, Redis, and mobile backend start."
        )
...
def print_offline_runtime_status(cfg: config.HarnessConfig) -> None:
    if cfg.provider_mode != "offline":
        return
    child_env = config.child_env_for(cfg)
    provider_credentials = sorted(key for key in config.CORE_PROVIDER_ENV if child_env.get(key))
    print("offline_hint: PROVIDER_MODE=offline active; outbound network is default-deny")
    print("egress_guard: enforced")
    print(f"provider_credentials_present: {', '.join(provider_credentials) if provider_credentials else 'none'}")
    print("offline_services: firestore, auth, redis, backend")
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    print_offline_runtime_status(cfg)
    if warnings:
        print("\nWarnings:")
...
def _start_services(cfg: config.HarnessConfig) -> None:
    _stop_unused_offline_services(cfg)
